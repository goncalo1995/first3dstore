...
- [x] `lib/modular-physical-grid.ts` defines `PhysicalWall`, `PhysicalRow`, `PhysicalColumn`.
...
- [x] AI fallback uses a curated restaurant template instead of converting planner hints into menu rows.
...
- [ ] Builder state migrates from `categories` to `walls`.
- [ ] Wall pills switch active wall without losing quote state.
- [ ] Titles are physical rows/columns, not category-only UI text.
- [ ] Add/remove wall.
- [ ] Add/remove row.
- [ ] Add/remove column.
...
- [ ] Global BOM aggregates all walls.
